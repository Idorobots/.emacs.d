...
(require 'htmlize)
;; For some reason Org adds additional newlines at the end of src blocks.
...
(setq org2blog/wp-confirm-post t)                 ; Always do this.
(setq org2blog/wp-use-wp-latex t)                 ; Me rickey.
...
(defun my-buffer-format-function (buffer-format)
  (format buffer-format
          (format-time-string "[%Y-%m-%d %a %H:%M]" (current-time))))

(setq org2blog/wp-buffer-format-function 'my-buffer-format-function)

(setq org2blog/wp-blog-alist '(("idorobots"
                                :url "http://idorobots.org/xmlrpc.php"
                                :username idorobots-username
                                :password idorobots-pass
...
;; Shopping list:
(defvar shopping-list-file "/tmp/shoppinglist.html")
    ...
    (org-map-entries (lambda ()
                       (save-excursion
                         (save-restriction
                           ...
                           (setq html
                                 (concat html
                                         (org-export-as-html 0 nil nil 'string t nil))))))
                     "+ShoppingList=\"true\""
                     'file 'archive 'comment)
    (with-temp-file shopping-list-file
      (let ((standard-output (current-buffer)))
        (princ "<!DOCTYPE html>")
        (princ "<html>")
        (princ "<head>")
        (princ "<meta http-equiv=\"content-type\" content=\"text/html;charset=UTF-8\" />")
                                        ;(princ "<link href=\"print.css\" rel=\"stylesheet\" type=\"text/css\" media=\"print\" />")
        (princ "<style type=\"text/css\" media=\"print\">")
        (princ "body > ul {
                  padding-left: 0;
                }
