...
(defvar gamify-munin-stats-file "/opt/stuff/gamify_munin_stats")
(defvar gamify-munin-config-file "/opt/stuff/gamify_munin_config")

(defun gamify-munin-stats ()
  (let* ((names (mapcar 'car gamify-stats-alist))
         (values (mapcar 'gamify-get-total-exp names))
         (level (car (reduce (lambda (stat acc)
                               (if (member (cdr stat) gamify-skip-levels)
                                   acc
                                 (if (< (car stat) (car acc))
                                     stat
                                   acc)))
                             gamify-stat-levels)))
         (nv-pairs (map 'list 'cons names values)))
    (list
     (cons (concat
            "graph_title Gamify Stats\n"
            "graph_info Gamify all the things for the Greater Good!\n"
            "graph_category kajtek\n"
            "graph_vlabel XP\n")
           (mapcar (lambda (nv)
                     (if (>= (cdr nv) level)
                         (concat
                          (format "gamify_%s.label %s\n" (car nv) (gamify-stat-name (car nv)))
                          ; (format "gamify_%s.type DERIVE\n" (car nv))
                          (format "gamify_%s.min 0\n" (car nv))
                          (format "gamify_%s.max 100000\n" (car nv)))))
                   nv-pairs))
     (mapcar (lambda (nv)
               (if (>= (cdr nv) level)
                   (format "gamify_%s.value %d\n" (car nv) (cdr nv))))
             nv-pairs))))

(defun gamify-munin-save-stats ()
  (let* ((stats-conf (gamify-munin-stats))
         (stats (cadr stats-conf))
         (config (car stats-conf)))
    (with-temp-buffer
      (dolist (conf config)
        (when conf
          (insert conf)))
      (write-file gamify-munin-config-file))
    (with-temp-buffer
      (dolist (stat stats)
        (when stat
          (insert stat)))
      (write-file gamify-munin-stats-file))))

(defadvice gamify-save-stats (after gamify-munin first () activate)
