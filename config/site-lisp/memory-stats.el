...
%R - Percentile main RAM usage (no buffers or cache).
%t - Total RAM in MB.
%c - Buffers/cache in MB.
%b - Buffers/cache in MB.
%f - Free main RAM in MB.
%F - Total free memory (with cache and buffers) in MB.
%u - Used memory in MB.
%U - Used memory (no cache or buffers) in MB.
...
(defun memory-stats ()
  ...
  (let ((stats (mapcar #'split-string
                 (remove-if (lambda (str) (string= str ""))
                   (split-string
                     (shell-command-to-string "free -m | tail -n2")
                     "\n")))))
    (mapcar (lambda (lst)
              (cons (car lst)
  ...
  (list
    ...
    (cons "F" (lambda (stats)
                (let* ((ram (assoc "Mem:" stats))
                       (total (nth 1 ram))
                       (used (nth 2 ram)))
                  (number-to-string (- total used)))))
    ; Used memory.
    (cons "u" (lambda (stats)
                (number-to-string (+ (nth 2 (assoc "Mem:" stats))
                                     (nth 5 (assoc "Mem:" stats))))))
    ; Used memory (no buffers/cache).
    (cons "U" (lambda (stats)
                (let* ((ram (assoc "Mem:" stats))
                       (used (nth 2 ram)))
                  (number-to-string used))))
    ; Total swap.
